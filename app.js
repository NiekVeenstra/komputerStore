...
//update displayed data on teh page
const updateData = () => {
...
//ask for loan amount and add loan when button is clicked
const handleGetLoan = () => {
  if (loanBalance !== 0) {
...
//add salary to paybalance and update data
const handleWork = () => {
  payBalance += salary;
  updateData();
};

//handle bank button and send money to bank if its there and update balances
const handleBank = () => {
  if (payBalance === 0) {
...
//update the balances and after that updates the data displayed on the screen
...
//adds a loan table to the screen when needed
const addLoanTable = () => {
...
//add repay button when some one has a loan
...
//removes teh pay button if there is no loan
...
//when repay button is clicked handles logic and subtract the money of the loan
...
//add the computers from the api to the dropdown menu
const addComputersToMenu = (computers) => {
  computers.forEach((x) => addComputerToMenu(x));
  // Show the features of the first computer in the menu
  ...
  //set data to the first one the first time the data gets loaded
  ...
  // Show the image of the first computer in the menu
...
//add computer to the dropdown menu
...
// Handle changes to the dropdown menu selection
...
// Update the features area with the selected computer's features
...
// Update the computer info area with the selected computer's information
...
// handle buy now button, if there is enough money buy the computer otherwise decline
const handleBuyNow = () => {
